...
async function getPost(commentId: string): Promise<Post | null> {
  try {
    const sql = await getPool()
    const comments = await sql.query(
      `
      SELECT 
        c.post_id_father as father_id
      FROM comments c
      WHERE c.id = $1
    `,
...
export default async function PostPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header />
      <div className="max-w-2xl mx-auto py-6 pt-12 px-4">
